...
public static class ComplexInjectionRegistrator
{
    ...
    extension(Type type)
    {
        ...
        private ServiceDescriptor CreateDescriptor(Type? Implementation, ServiceLifetime Mode)
        {
            var service_type = Implementation ?? type;
            ...
            var get_service = typeof(IServiceProvider).GetMethod(nameof(IServiceProvider.GetService), [typeof(Type)])
            ...
            var ctor_parameters = ctor.GetParameters()
                ...
                .Select(p =>
                {
                    ...
                    var type1 = Expression.Constant(parameter_type);             //    превращаем в выражение
                    // Вызываем sp.GetRequiredService(parameter_type)
                    var call = required
                        ? Expression.Call(get_required_service, sp, type1)
                        : Expression.Call(sp, get_service, type1);   
            ...
            var properties = service_type.GetProperties(inst_public | BindingFlags.SetProperty)
                ...
                .Select(MemberBinding (p) =>
                {
                    ...
                    var type1 = Expression.Constant(property_type);           //    превращаем в выражение
            ...
            var fields = service_type.GetFields(inst_public)
                ...
                .Select(MemberBinding (f) =>
                {
                    ...
                    var type1 = Expression.Constant(field_type);             //    превращаем в выражение
                    // Вызываем sp.GetRequiredService(property_type)
                    var obj = required
                        ? Expression.Call(get_required_service, sp, type1)
                        : Expression.Call(sp, get_service, type1);
                ...
                .Select(Expression (InitMethod) =>
                {
                    ...
                    var parameters = InitMethod
                        ...
                        .Select(parameter =>
                        {
                            ...
                            var type1 = Expression.Constant(parameter_type);   //   формируем из него выражение
                    ...
                    Expression service_impl = type == service_type 
                        ? result 
            ...
            var body = methods.Length > 0
                ? Expression.Block(     // формируем блок с последовательностью вызовов этих методов
                    [result],                                               // определяем переменные внутри блока - объект сервиса
